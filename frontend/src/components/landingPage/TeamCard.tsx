...
import styles from "..";
const TeamCard = ({id, imgUrl,title, index, active, handleClick}: {id:string, imgUrl:string,title:string, index:number, active:string, handleClick:any}) => (
  <motion.div variants={fadeIn('right', 'spring', index * 0.5,0.75)} className={`relative ${active === id ? 'lg:flex-[3.5] flex-[10]':'lg:flex-[0.5] flex-[2]'} flex items-center justify-center min-w-[170px] h-[700px] transition-[flex] duration-[0.7s] ease-out-flex cursor-pointer`} onClick={()=> handleClick(id)}>
    ...
    (
      <div className="absolute bottom-0 p-8 justify-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-[24px]">
        ...
        <p className="font-normal text-[16px] leading-[20px] text-white uppercase">Check Github</p>
