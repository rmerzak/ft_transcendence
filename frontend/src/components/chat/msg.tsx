...
const Msg = () => {
  return (
    <div>
      <div className="mt-6">
        <h1 className="text-white text-xl text-center font-inter">Recent</h1>
        <div className="flex justify-center mt-2">
          <div className="mb-4 border-b border-white w-16"></div>
        </div>
      </div>

      <div className="mx-auto w-3/4 scroll-y-auto max-h-[300px] my-2 text-white">
        {msg.map((msg) => (
          <div
            key={msg.name}
            className="flex justify-between items-center my-[10px] rounded-md font-inter bg-[#5D5959]/50 hover:bg-[#5D5959]/100"
          >
            <div className="flex items-center space-x-3 p-3 hover:cursor-pointer w-[95%] h-full active:p-[10px]">
              <Image
                src={msg.avatar}
                alt={msg.name}
                width={25}
